...
then command channel join prison write
then deny
...
# This rule only executes when player is not in the staff channel for reading.
match ^(\/staffchat)
dont verbose
ignore channel staff write
then command channel join staff write
then deny

# This rule only executes when player is in the staff channel for writing.
match ^(\/staffchat)
dont verbose
